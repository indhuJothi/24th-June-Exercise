...
else
{
    ...
    if((nameErr||dobErr||emailErr||mobileErr||radioErr||fromErr||toErr||countErr||dateErr||retErr)==true)
    {
        ...
        var datas=` you have entered the following details:<br>
        Name: ${name}<br>
        Email: ${email}<br>
        Mobile: ${mobile}<br>
        DOB: ${birth}<br>
        Traveltype: ${oneway}<br>
        From: ${from}<br>
        To: ${to}<br>
        No of Passengers: ${count}<br>
        Ceat Type: ${bustype}<br>
        start Date: ${Date1}`
        if(document.getElementById('onewaytraveltype').checked)
       {
        document.querySelector('body').style.backgroundImage='none';
         document.getElementById('form').style.display='none';  
         document.getElementById('pa').innerHTML=datas;
      }
    else{
        var datass=`${datas}<br>End Date: ${Date2}`;
        document.querySelector('body').style.backgroundImage='none';
         document.getElementById('form').style.display='none';  
         document.getElementById('pa').innerHTML=datass;
